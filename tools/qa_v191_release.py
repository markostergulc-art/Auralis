...
APK=ROOT/'manual_apk/out/BrainRelaxSoundPerception_v1.9.1.apk'
HTML=ROOT/'manual_apk/assets/index.html'
BASEZIP=Path('/mnt/data/brain_v191/BrainRelaxSoundPerception_v1.9.0_source.zip')
checks=[]
...
with zipfile.ZipFile(APK) as z:
    ...
    ck('Version 1.9.1',attrs.get('manifest.versionName')=='1.9.1')
    ck('VersionCode 15',attrs.get('manifest.versionCode')==15)
...
ck('Version shown EN/DE/HR',html.count('v1.9.1')>=3)
...
with zipfile.ZipFile(BASEZIP) as z:
    ...
    def catalog(s):
        x=s.split('const BUILTIN_PRESETS=',1)[1]; return re.split(r';\nconst (?:PRESET_PURPOSES|BIFAVKEY)',x,1)[0]
...
ck('Full source version 1.9.1','versionName = "1.9.1"' in build and 'versionCode = 15' in build)
# signature
