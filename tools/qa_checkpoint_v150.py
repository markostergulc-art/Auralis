...
APK=ROOT/'manual_apk/out/BrainRelaxSoundPerception_v1.5.0.apk'
...
with zipfile.ZipFile(APK) as z:
    ...
    ck('Version name',attrs.get('manifest.versionName')=='1.5.0',str(attrs.get('manifest.versionName')))
    ck('Version code',attrs.get('manifest.versionCode')==9,str(attrs.get('manifest.versionCode')))
...
Path('/tmp/brain_v150_check.js').write_text(js)
p=subprocess.run(['node','--check','/tmp/brain_v150_check.js'],capture_output=True,text=True)
