...
APK=ROOT/'manual_apk/out/BrainRelaxSoundPerception_v1.8.0.apk'
...
def parse_dex(d):
    ...
    ck('Native diagnostics bridge',all(x in strings for x in ['startPcm','stopPcm','setPcmVolume','getNativeSampleRate','getCpuTimeMs','getMemoryPssKb','getOutputDeviceType']))
...
with zipfile.ZipFile(APK) as z:
    ...
    ck('Version name',attrs.get('manifest.versionName')=='1.8.0',str(attrs.get('manifest.versionName')))
    ck('Version code',attrs.get('manifest.versionCode')==12,str(attrs.get('manifest.versionCode')))
...
Path('/tmp/brain_v160_check.js').write_text(js)
p=subprocess.run(['node','--check','/tmp/brain_v160_check.js'],capture_output=True,text=True)
...
ck('Preset schema v2','const PRESET_SCHEMA=2' in html and 'presetSnapshot(name)' in html)
ck('N-of-1 experiment','id="experiment"' in html and 'selected.length<2||selected.length>5' in html and 'g.length<5' in html)
ck('Headphone safety','safeListeningText' in html and 'routeSafetyGain=.5' in html and 'getOutputDeviceType' in html)
ck('Learn center','data-open="learn"' in html and 'const LEARN=' in html)
ck('Localization extensions retained','diagRoute' in html and 'openLearn' in html and 'learnLead' in html)
ck('Accessibility semantics','aria-live="polite"' in html and 'function updateAccessibility()' in html)
ck('Expanded runtime diagnostics','diagBinaural' in html and 'diagItd' in html and 'diagMixPeak' in html)
ck('PCM payload cache','function audioPayload()' in html and 'payloadCache' in html)
ck('Volume safety gain without rebuild','Android.setPcmVolume((state.volume/100)*routeSafetyGain)' in html)
ck('Inactive layer guards',all(x in html for x in ['if(!state.ambientEnabled)return','if(!state.binauralEnabled)return','if(!state.monauralEnabled)return','if(!state.isoEnabled)return']))
ck('40 built-in presets','const BUILTIN_PRESETS=' in html and len(re.findall(r'\"id\":\"[^\"]+\"', html.split('const BUILTIN_PRESETS=',1)[1].split(';\nconst BIFAVKEY',1)[0]))==40)
ck('Built-in category filters',all(x in html for x in ['focusCategory','relaxCategory','breathingCategory','sleepCategory','spatialCategory','beatsCategory','experimentalCategory']))
ck('Built-in immutable split','builtInPresets' in html and 'myPresets' in html and 'copyBuiltIn' in html)
ck('Built-in favorites','BIFAVKEY' in html and 'favSet' in html)
ck('Built-in current preset localization',"currentPreset.startsWith('@')" in html)
ck('Built-in timer preserved','const timer=state.timerMinutes' in html)
ck('Built-in reset unspecified layers',"state={...defaults,...p.cfg,timerMinutes:timer,theme,language,currentPreset:'@'+p.id}" in html)
ck('Cardiorespiratory caveat',all(x in html for x in ['does not measure heart rhythm','misst keinen Herzrhythmus','ne mjeri srčani ritam']))
ck('Respiratory caveat',all(x in html for x in ['not treatment for asthma','keine Behandlung von Asthma','ne liječenje astme']))
ck('Binaural non-theta caveat','does not prove that the brain enters a 6 Hz theta state' in html)
ck('Schumann physical caveat','not physically equivalent to electromagnetic Schumann resonance' in html)
ck('528 medical caveat','does not claim DNA repair' in html)
