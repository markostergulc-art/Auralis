...
APK=ROOT/'manual_apk/out/BrainRelaxSoundPerception_v1.3.0.apk'
HTML=ROOT/'manual_apk/assets/index.html'
OLD=Path('/mnt/data/BrainRelaxSoundPerception_v1.2.1_HuaweiAudioFix.apk')
checks=[]
...
with zipfile.ZipFile(APK) as z:
    ...
    ck('Version name', attrs.get('manifest.versionName')=='1.3.0',str(attrs.get('manifest.versionName')))
    ck('Version code', attrs.get('manifest.versionCode')==6,str(attrs.get('manifest.versionCode')))
...
ck('Main font matches Volume window scale', 'font-size:1.03rem' in html and '.action{font-size:1.03rem}' in html)
ck('Bottom navigation pinned', '.actions{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:auto' in html)
ck('Volume icon restored', 'data-open="volumeWin"><span class="ico">🔊</span>' in html)
ck('Timer icon enlarged', 'timerIcon' in html and 'font-size:1.90rem' in html)
...
ck('Pulse Duration renamed', "requestedPulse:'Pulse Duration'" in html and '<strong data-i18n="requestedPulse">Pulse Duration</strong>' in html)
ck('Pulse Duration 40-90', 'id="dur" type="range" min="40" max="90"' in html and 'state.durationUs=Math.round(clamp(Number(state.durationUs)||55,40,90))' in html)
ck('Random renamed to Random Pulse Timing', "random:'Random Pulse Timing'" in html and '<strong data-i18n="random">Random Pulse Timing</strong>' in html)
ck('Random L/R Delay 30-90', 'id="delay" type="range" min="30" max="90"' in html and 'state.stereoDelayUs=Math.round(clamp(Number(state.stereoDelayUs)||30,30,90))' in html)
ck('Random L/R generated from 30us minimum', 'const requestedDelay=30+Math.random()*Math.max(0,state.stereoDelayUs-30)' in html)
ck('Preset Update option', 'data-update="${i}"' in html and "t('presetUpdated')" in html)
...
ck('Timer/Presets/Settings icons present', html.count('actionIcon') >= 3)
# i18n parity
...
if OLD.exists():
    ...
    oldc,newc=cert(OLD),cert(APK); ck('Signing continuity v1.2.1 -> v1.3.0',oldc==newc and oldc is not None,newc or '')
# v1 jar signature
