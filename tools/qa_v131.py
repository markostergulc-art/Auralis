...
APK=ROOT/'manual_apk/out/BrainRelaxSoundPerception_v1.3.1.apk'
HTML=ROOT/'manual_apk/assets/index.html'
OLD=Path('/mnt/data/brain_v131/BrainRelaxSoundPerception_v1.3.0.apk')
checks=[]
...
with zipfile.ZipFile(APK) as z:
    ...
    ck('Version name', attrs.get('manifest.versionName')=='1.3.1',str(attrs.get('manifest.versionName')))
    ck('Version code', attrs.get('manifest.versionCode')==7,str(attrs.get('manifest.versionCode')))
    ...
    ck('Black startup window theme', attrs.get('application.theme')==0x01030009,hex(attrs.get('application.theme',0)))
...
ck('App title removed above Play/Stop', '<div class="title" data-i18n="appTitle">' not in html)
ck('Play/Stop moved slightly lower', '.hero{display:flex;flex-direction:column;align-items:center;padding:34px 0 6px}' in html)
ck('Current Preset centered area', 'class="presetCenter"' in html and '.presetCenter{flex:1;display:flex;align-items:center;justify-content:center' in html)
ck('Volume/Pulse above bottom actions', html.index('class="mainWindows"') < html.index('class="actions"'))
ck('Bottom Timer/Presets/Settings retained', '.actions{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:0;padding-top:0}' in html)
ck('Timer icon enlarged again', 'font-size:2.55rem' in html and 'width:46px;height:46px' in html)
ck('All non-first windows full screen', '.modal:not(.first) .sheet{height:100vh;height:100dvh;max-height:100vh;max-height:100dvh' in html)
ck('Timer buttons left center right', all(x in html for x in ['class="sheetBtn left"','class="sheetBtn center"','class="sheetBtn primary right"']))
...
ck('Random Pulse Timing retained', "random:'Random Pulse Timing'" in html)
ck('Random L/R Delay 1-100 ms', 'id="delay" type="range" min="1" max="100"' in html and "state.stereoDelayUs=Math.round(clamp(Number(state.stereoDelayUs)||30,1,100))" in html)
ck('Random L/R generated from 1 ms minimum', 'const requestedDelay=1+Math.random()*Math.max(0,state.stereoDelayUs-1)' in html)
ck('Delay frame conversion uses milliseconds', 'Math.round(sr*ms/1e3)' in html)
ck('Diagnostics display milliseconds', "maxDelay.toFixed(2)+' ms'" in html)
ck('Preset Update option retained', 'data-update="${i}"' in html and "t('presetUpdated')" in html)
ck('Huawei audible waveform retained', 'Math.exp(-0.6*i)' in html and 'baseAmp=.75' in html)
...
ck('Translation parity', all(keys<=set(v) for v in i18n.values()) and len({frozenset(v) for v in i18n.values()})==1,f'refs={len(keys)}')
# cert continuity
if OLD.exists():
    ...
    oldc,newc=cert(OLD),cert(APK); ck('Signing continuity v1.3.0 -> v1.3.1',oldc==newc and oldc is not None,newc or '')
# v1 jar signature
